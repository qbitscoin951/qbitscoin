...
/** Call node.updateRwSetting() with Qbitscoin 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Qbitscoin 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/qbitscoin/qbitscoin/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because qbitscoin 22.x will not try to read these.
...
//! Convert enabled/size values to qbitscoin -prune setting.
...
//! Get pruning enabled value to show in GUI from qbitscoin -prune setting.
...
//! Get pruning size value to show in GUI from qbitscoin -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayQbitscoinUnit")) {
        settings.setValue("DisplayQbitscoinUnit", QVariant::fromValue(QbitscoinUnit::QTC));
    }
    QVariant unit = settings.value("DisplayQbitscoinUnit");
    if (unit.canConvert<QbitscoinUnit>()) {
        m_display_qbitscoin_unit = unit.value<QbitscoinUnit>();
    } else {
        m_display_qbitscoin_unit = QbitscoinUnit::QTC;
        settings.setValue("DisplayQbitscoinUnit", QVariant::fromValue(m_display_qbitscoin_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if qbitscoin.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_qbitscoin_unit);
...
{
    if (new_unit.isNull() || new_unit.value<QbitscoinUnit>() == m_display_qbitscoin_unit) return;
    m_display_qbitscoin_unit = new_unit.value<QbitscoinUnit>();
    QSettings settings;
    settings.setValue("DisplayQbitscoinUnit", QVariant::fromValue(m_display_qbitscoin_unit));
    Q_EMIT displayUnitChanged(m_display_qbitscoin_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/qbitscoin/qbitscoin/pull/8273
    ...
    // (https://github.com/qbitscoin-core/gui/issues/567).
