...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<QbitscoinUnit>("QbitscoinUnit");
#else
    qRegisterMetaType<QbitscoinUnit>("QbitscoinUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. qbitscoin_de.qm (shortcut "de" needs to be defined in qbitscoin.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. qbitscoin_de_DE.qm (shortcut "de_DE" needs to be defined in qbitscoin.qrc)
...
static const char* qt_argv = "qbitscoin-qt";

QbitscoinApplication::QbitscoinApplication()
...
void QbitscoinApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the QbitscoinApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", QbitscoinGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void QbitscoinApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool QbitscoinApplication::createOptionsModel(bool resetSettings)
{
...
void QbitscoinApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new QbitscoinGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &QbitscoinGUI::quitRequested, this, &QbitscoinApplication::requestShutdown);
...
void QbitscoinApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void QbitscoinApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool QbitscoinApplication::baseInitialize()
{
    return node().baseInitialize();
}

void QbitscoinApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &QbitscoinApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &QbitscoinApplication::handleRunawayException);
    connect(this, &QbitscoinApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &QbitscoinApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void QbitscoinApplication::parameterSetup()
{
...
void QbitscoinApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void QbitscoinApplication::requestInitialize()
{
...
void QbitscoinApplication::requestShutdown()
{
...
void QbitscoinApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (success) {
        ...
        // qbitscoin: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &QbitscoinGUI::handlePaymentRequest);
            connect(window, &QbitscoinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void QbitscoinApplication::handleRunawayException(const QString &message)
{
...
void QbitscoinApplication::handleNonFatalException(const QString& message)
{
...
WId QbitscoinApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool QbitscoinApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", QbitscoinGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(qbitscoin);
    Q_INIT_RESOURCE(qbitscoin_locale);
    ...
    for (int i = 1; i < argc; i++) {
...
#ifdef ENABLE_WALLET
        if (arg.startsWith(QBITSCOIN_IPC_PREFIX, Qt::CaseInsensitive)) {
            invalid_token &= false;
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see qbitscoin-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, PACKAGE_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see qbitscoin-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse qbitscoin.conf, determine network, switch to network specific
    ...
    // qbitscoin: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
