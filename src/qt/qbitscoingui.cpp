...
const std::string QbitscoinGUI::DEFAULT_UIPLATFORM =
...
QbitscoinGUI::QbitscoinGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    if(enableWallet)
    {
        ...
        connect(walletFrame, &WalletFrame::createWalletButtonClicked, this, &QbitscoinGUI::createWallet);
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &QbitscoinGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &QbitscoinGUI::showModalOverlay);
...
void QbitscoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Qbitscoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and qbitscoin: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &QbitscoinGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &QbitscoinGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &QbitscoinGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Qbitscoin addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Qbitscoin addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Qbitscoin Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Qbitscoin Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a qbitscoin: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Qbitscoin command-line options").arg(PACKAGE_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &QbitscoinGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &QbitscoinGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &QbitscoinGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &QbitscoinGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &QbitscoinGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &QbitscoinGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const std::pair<const std::string, bool>& i : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &QbitscoinGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &QbitscoinGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, this, &QbitscoinGUI::createWallet);
        connect(m_close_all_wallets_action, &QAction::triggered, [this] {
            m_wallet_controller->closeAllWallets(this);
        });
        connect(m_migrate_wallet_action, &QAction::triggered, [this] {
            auto activity = new MigrateWalletActivity(m_wallet_controller, this);
            connect(activity, &MigrateWalletActivity::migrated, this, &QbitscoinGUI::setCurrentWallet);
            activity->migrate(walletFrame->currentWalletModel());
        });
        connect(m_mask_values_action, &QAction::toggled, this, &QbitscoinGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &QbitscoinGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &QbitscoinGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &QbitscoinGUI::showDebugWindow);
}

void QbitscoinGUI::createMenuBar()
{
    appMenuBar = menuBar();
...
void QbitscoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &QbitscoinGUI::setCurrentWalletBySelectorIndex);
...
void QbitscoinGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &QbitscoinGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &QbitscoinGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &QbitscoinGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &QbitscoinGUI::showProgress);
...
#ifdef ENABLE_WALLET
void QbitscoinGUI::enableHistoryAction(bool privacy)
{
...
void QbitscoinGUI::setWalletController(WalletController* wallet_controller, bool show_loading_minimized)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &QbitscoinGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &QbitscoinGUI::removeWallet);
...
WalletController* QbitscoinGUI::getWalletController()
{
    return m_wallet_controller;
}

void QbitscoinGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &QbitscoinGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &QbitscoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &QbitscoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &QbitscoinGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &QbitscoinGUI::incomingTransaction);
    connect(this, &QbitscoinGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void QbitscoinGUI::removeWallet(WalletModel* walletModel)
{
...
void QbitscoinGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void QbitscoinGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void QbitscoinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void QbitscoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void QbitscoinGUI::createTrayIcon()
{
...
void QbitscoinGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &QbitscoinGUI::toggleHidden);
...
void QbitscoinGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void QbitscoinGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void QbitscoinGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void QbitscoinGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void QbitscoinGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void QbitscoinGUI::openClicked()
{
...
void QbitscoinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void QbitscoinGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void QbitscoinGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void QbitscoinGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void QbitscoinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void QbitscoinGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void QbitscoinGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void QbitscoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Qbitscoin network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void QbitscoinGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void QbitscoinGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void QbitscoinGUI::updateHeadersSyncProgressLabel()
{
...
void QbitscoinGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void QbitscoinGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &QbitscoinGUI::quitRequested);
...
void QbitscoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void QbitscoinGUI::createWallet()
{
#ifdef ENABLE_WALLET
#ifndef USE_SQLITE
    ...
    connect(activity, &CreateWalletActivity::created, this, &QbitscoinGUI::setCurrentWallet);
    connect(activity, &CreateWalletActivity::created, rpcConsole, &RPCConsole::setCurrentWallet);
    activity->create();
#endif // ENABLE_WALLET
}

void QbitscoinGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void QbitscoinGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &QbitscoinGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &QbitscoinGUI::show);
                e->ignore();
            }
        }
    }
...
void QbitscoinGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void QbitscoinGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void QbitscoinGUI::incomingTransaction(const QString& date, QbitscoinUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(QbitscoinUnits::formatWithUnit(unit, amount, true));
...
void QbitscoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void QbitscoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool QbitscoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool QbitscoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void QbitscoinGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void QbitscoinGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void QbitscoinGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void QbitscoinGUI::updateProxyIcon()
{
    std::string ip_port;
...
void QbitscoinGUI::updateWindowTitle()
{
...
void QbitscoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void QbitscoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void QbitscoinGUI::detectShutdown()
{
...
void QbitscoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void QbitscoinGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(QbitscoinGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = QbitscoinGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void QbitscoinGUI::subscribeToCoreSignals()
{
...
void QbitscoinGUI::unsubscribeFromCoreSignals()
{
...
bool QbitscoinGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<QbitscoinUnit> units = QbitscoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const QbitscoinUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, QbitscoinUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const QbitscoinUnit u : QbitscoinUnits::availableUnits()) {
        menu->addAction(QbitscoinUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(QbitscoinUnit newUnits)
{
    setText(QbitscoinUnits::longName(newUnits));
}
