...
const char * const QBITSCOIN_CONF_FILENAME = "qbitscoin.conf";
const char * const QBITSCOIN_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //qbitscoin-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", QBITSCOIN_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Qbitscoin
    // macOS: ~/Library/Application Support/Qbitscoin
    // Unix-like: ~/.qbitscoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Qbitscoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Qbitscoin";
#else
    // Unix-like
    return pathRet / ".qbitscoin";
#endif
#endif
}
