# User-space, Statically Defined Tracing (USDT) for Qbitscoin Core

Qbitscoin Core includes statically defined tracepoints to allow for more
...
**Note**: Qbitscoin Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Qbitscoin Core
...
Multiple tools can list the available tracepoints in a `qbitscoind` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Qbitscoin Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/qbitscoind
stap net        outbound_message 0x0000000000107c05 /src/qbitscoind
stap validation block_connected  0x00000000002fb10c /src/qbitscoind
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Qbitscoin Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/qbitscoind | grep NT_STAPSDT -A 4 -B 2
