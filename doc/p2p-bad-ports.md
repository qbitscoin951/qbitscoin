When Qbitscoin Core automatically opens outgoing P2P connections, it chooses
...
A malicious actor may gossip an address:port where no Qbitscoin node is listening,
or one where a service is listening that is not related to the Qbitscoin network.
As a result, this service may occasionally get connection attempts from Qbitscoin
nodes.

"Bad" ports are ones used by services which are usually not open to the public
and usually require authentication. A connection attempt (by Qbitscoin Core,
trying to connect because it thinks there is a Qbitscoin node on that
...
Below is a list of "bad" ports which Qbitscoin Core avoids when choosing a peer to
...
[pull/23306](https://github.com/qbitscoin/qbitscoin/pull/23306#issuecomment-947516736)

[pull/23542](https://github.com/qbitscoin/qbitscoin/pull/23542)
