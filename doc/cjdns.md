# CJDNS support in Qbitscoin Core

It is possible to run Qbitscoin Core over CJDNS, an encrypted IPv6 network that
...
done outside of Qbitscoin Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Qbitscoin network and individual nodes.
...
before it will work with your Qbitscoin Core node. This is described in steps
...
## Run Qbitscoin Core with CJDNS

Once you are connected to the CJDNS network, the following Qbitscoin Core
...
When enabled, this option tells Qbitscoin Core that it is running in an
...
IPv6 local network. This helps Qbitscoin Core perform better address management:
...
CJDNS support was added to Qbitscoin Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `qbitscoin-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/qbitscoin/qbitscoin/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Qbitscoin Core

There are several ways to see your CJDNS address in Qbitscoin Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `qbitscoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `qbitscoin-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `qbitscoin-cli help getnodeaddresses` for details.
