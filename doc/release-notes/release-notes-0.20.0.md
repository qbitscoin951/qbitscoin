...
Qbitscoin Core version 0.20.0 is now available from:

  <https://qbitscoincore.org/bin/qbitscoin-core-0.20.0/>
...
using the Linux kernel, macOS 10.12+, and Windows 7 and newer.  Qbitscoin
Core should also work on most other Unix-like systems but is not as
frequently tested on them.  It is not recommended to use Qbitscoin Core on
unsupported systems.

From Qbitscoin Core 0.20.0 onwards, macOS versions earlier than 10.12 are no
longer supported. Additionally, Qbitscoin Core does not yet change appearance
...
- Instead of running `make` simply, you should instead run
  `QBITSCOIN_GENBUILD_NO_GIT=1 make`.
...
#### Removal of BIP61 reject network messages from Qbitscoin Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Qbitscoin Core version 0.18.0.
...
- Testing or debugging of implementations of the Qbitscoin P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Qbitscoin Core. Qbitscoin Core logs debug messages
...
- OpenSSL is no longer used by Qbitscoin Core.  (#17265)

- BIP70 support has been fully removed from Qbitscoin Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/qbitscoin/qbitscoin/issues/16599),
  [PR #16702](https://github.com/qbitscoin/qbitscoin/pull/16702), and the
  `qbitscoind help` for more information.  This option is experimental and
...
- All custom settings configured when Qbitscoin Core starts are now
...
- The "Start Qbitscoin Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://qbitscoincore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple Qbitscoin Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Qbitscoin Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/qbitscoin-core/HWI) for
  signing.  Future versions of Qbitscoin Core should support a GUI option
...
- Qbitscoin Core's automatically-generated source code documentation is
  now available at https://doxygen.qbitscoincore.org.  (#17596)
...
- The `qbitscoin-cli` utility used with the `-getinfo` parameter now
...
- The `qbitscoin-cli` utility now accepts a `-stdinwalletpassphrase`
...
- Command line options prefixed with main/test/regtest network names like
  `-main.port=8663` `-test.server=1` previously were allowed but ignored. Now
...
- #17068 Always generate `qbitscoinstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_qbitscoin-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBQBITSCOIN_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_qbitscoin` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBQBITSCOIN_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for qbitscoin-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate qbitscoin schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest qbitscoin.conf (sdaftuar)
...
- #18022 Fix appveyor `test_qbitscoin` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (qbitscoin-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone qbitscoin-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Qbitscoin Core files (hebasto)
...
- #17119 Fix broken qbitscoin-cli examples (andrewtoth)
...
- #17503 Remove bitness from qbitscoin-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.qbitscoincore.org (laanwj)
...
- #17648 Rename wallet-tool references to qbitscoin-wallet (hel-o)
...
- #16115 On qbitscoind startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from qbitscoin-cli -getinfo (malevolent)
