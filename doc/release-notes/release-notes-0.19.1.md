...
Qbitscoin Core version 0.19.1 is now available from:

  <https://qbitscoincore.org/bin/qbitscoin-core-0.19.1/>
...
From Qbitscoin Core 0.17.0 onwards, macOS versions earlier than 10.10 are no
longer supported, as Qbitscoin Core is now built using Qt 5.9.x which requires
macOS 10.10+. Additionally, Qbitscoin Core does not yet change appearance when
...
- #17488 fix "qbitscoind already running" warnings on macOS (fanquake)
