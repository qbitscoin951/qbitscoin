Qbitscoin Core version 0.11.0 is now available from:

  <https://qbitscoin.org/bin/qbitscoin-core-0.11.0/>
...
For example, add the following to `qbitscoin.conf`:
...
related to the blockchain in the qbitscoin system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Qbitscoin Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Qbitscoin Core will 
...
For an example script see [qbitscoin-submittx](https://github.com/laanwj/qbitscoin-submittx).
...
- #5334 `cf87536` libqbitscoinconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new qbitscoin-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only QbitscoinAmountField
- #5753 `0f44672` Add qbitscoin logo to about screen
...
- #5809 `46bfbe7` Add qbitscoin-cli man page
...
- #5849 `41259ca` contrib/init/qbitscoind.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Qbitscoin-Qt.app to Qbitscoin-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for qbitscoin-tx)
