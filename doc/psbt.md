# PSBT Howto for Qbitscoin Core

Since Qbitscoin Core 0.17, an RPC interface exists for Partially Signed Qbitscoin
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/qbitscoin/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for Qbitscoin transactions that are not fully signed
...
#### Multisig with multiple Qbitscoin Core instances
...
Qbitscoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `qbitscoin-cli` is used.
...
Later, when *V* QTC has been received on *Amulti*, and Bob and Carol want to
