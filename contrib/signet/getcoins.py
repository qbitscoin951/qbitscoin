...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing qbitscoin-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='qbitscoin-cli', help='qbitscoin-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Qbitscoin address to which the faucet should send')
...
parser.add_argument('qbitscoin_cli_args', nargs='*', help='Arguments to pass on to qbitscoin-cli (default: -signet)')

args = parser.parse_args()

if args.qbitscoin_cli_args == []:
    args.qbitscoin_cli_args = ['-signet']


def qbitscoin_cli(rpc_command_and_params):
    argv = [args.cmd] + args.qbitscoin_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = qbitscoin_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = qbitscoin_cli(['getnewaddress', 'faucet', 'bech32'])
