...
""" Example logging Qbitscoin Core mempool events using the mempool:added,
...
def main(qbitscoind_path):
    qbitscoind_with_usdts = USDT(path=str(qbitscoind_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    qbitscoind_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    qbitscoind_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    qbitscoind_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    qbitscoind_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[qbitscoind_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/qbitscoind")
        exit(1)
