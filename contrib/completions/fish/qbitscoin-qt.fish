...
complete --command qbitscoin-qt --no-files

# Extract options
function __fish_qbitscoinqt_get_options
...
complete \
    --command qbitscoin-qt \
    --arguments "(__fish_qbitscoinqt_get_options)"
# Enable file completions only if the commandline now contains a `*.=` style option
complete -c qbitscoin-qt \
...
complete \
    --command qbitscoin-qt \
    --arguments "(__fish_qbitscoinqt_get_options --nofiles)"
