...
"""Script for verifying Qbitscoin Core release binaries.

This script attempts to download the sum file SHA256SUMS and corresponding
signature file SHA256SUMS.asc from qbitscoincore.org and qbitscoin.org and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/qbitscoin-core/guix.sigs/tree/main/builder-keys
...
HOST1 = "https://qbitscoincore.org"
HOST2 = "https://qbitscoin.org"
VERSIONPREFIX = "qbitscoin-core-"
...
def verify_shasums_signature(
   ...
   ReturnCode, list[SigData], list[SigData], list[SigData], list[SigData]
    ...
    # which pubkeys within the Qbitscoin community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"qbitscoin_verify_binaries.{args.version}"
    ...
    # remove binaries that are known not to be hosted by qbitscoincore.org
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the qbitscoin release to download; of the format '
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes qbitscoin.org lags behind qbitscoincore.org.)')
    )
