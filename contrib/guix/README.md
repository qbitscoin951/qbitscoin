# Bootstrappable Qbitscoin Core Builds

This directory contains the files necessary to perform bootstrappable Qbitscoin
Core builds.
...
To build Qbitscoin Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Qbitscoin Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/qbitscoin-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/qbitscoin-core/guix.sigs).
