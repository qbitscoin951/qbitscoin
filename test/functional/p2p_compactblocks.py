...
# TestP2PConn: A peer we use to send messages to qbitscoind, and store responses.
...
class CompactBlocksTest(QbitscoinTestFramework):
    ...
    # This test actually causes qbitscoind to (reasonably!) disconnect us, so do this last.
    ...
    # Test that qbitscoind requests compact blocks when we announce new blocks
    ...
    def test_incorrect_blocktxn_response(self, test_node):
        ...
        # Note that it's possible for qbitscoind to be smart enough to know we're
    ...
    def test_getblocktxn_handler(self, test_node):
        node = self.nodes[0]
        # qbitscoind will not send blocktxn responses for blocks whose height is
