#!/usr/bin/env python3
# Copyright (c) 2014-2022 The Qbitscoin Core developers
...
TMPDIR_PREFIX = "qbitscoin_func_test_"
...
class QbitscoinTestMetaClass(type):
    """Metaclass for QbitscoinTestFramework.

    Ensures that any attempt to register a subclass of `QbitscoinTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'QbitscoinTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("QbitscoinTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("QbitscoinTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class QbitscoinTestFramework(metaclass=QbitscoinTestMetaClass):
    """Base class for a qbitscoin test script.

    Individual qbitscoin test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave qbitscoinds and test.* datadir on exit or error")
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop qbitscoinds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use qbitscoin-cli instead of RPC for all commands")
    ...
    def set_binary_paths(self):
        ...
        binaries = {
            "qbitscoind": ("qbitscoind", "QBITSCOIND"),
            "qbitscoin-cli": ("qbitscoincli", "QBITSCOINCLI"),
            "qbitscoin-util": ("qbitscoinutil", "QBITSCOINUTIL"),
            "qbitscoin-wallet": ("qbitscoinwallet", "QBITSCOINWALLET"),
        }
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: qbitscoinds were not stopped and may still be running")
        ...
        rpc_logger = logging.getLogger("QbitscoinRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'qbitscoind', self.options.qbitscoind) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'qbitscoin-cli', self.options.qbitscoincli) for v in versions]
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a qbitscoind test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple qbitscoind test nodes"""
        for node in self.nodes:
    ...
    def create_outpoints(self, node, *, outputs):
        """Send funds to a given list of `{address: amount}` targets using the qbitscoind
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as qbitscoind's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    qbitscoind=self.options.qbitscoind,
                    qbitscoin_cli=self.options.qbitscoincli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in qbitscoin.conf
    ...
    def skip_if_no_qbitscoind_tracepoints(self):
        """Skip the running test if qbitscoind has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("qbitscoind has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_qbitscoind_zmq(self):
        """Skip the running test if qbitscoind has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("qbitscoind has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if qbitscoin-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("qbitscoin-wallet has not been compiled")

    def skip_if_no_qbitscoin_util(self):
        """Skip the running test if qbitscoin-util has not been compiled."""
        if not self.is_qbitscoin_util_compiled():
            raise SkipTest("qbitscoin-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if qbitscoin-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("qbitscoin-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether qbitscoin-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether qbitscoin-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_qbitscoin_util_compiled(self):
        """Checks whether qbitscoin-util was compiled."""
        return self.config["components"].getboolean("ENABLE_QBITSCOIN_UTIL")
